...
import { 
  CheckCircle2, Sparkles, ArrowRight, Loader2, ImageIcon, AlertCircle, Send, XCircle, Clock
} from 'lucide-react';
    ...
    <div className="space-y-8 text-right animate-in fade-in" dir="rtl">
      <div className="flex flex-col md:flex-row justify-between items-center gap-6">
        <div className="text-right">
          ...
          <p className="text-slate-500 font-bold mt-1">إدارة ومعالجة البلاغات المصرفية الواردة</p>
        </div>
        <Tabs value={activeStatusFilter} onValueChange={setActiveStatusFilter} className="bg-slate-100 p-1 rounded-full h-auto no-scrollbar overflow-x-auto">
          <TabsList className="bg-transparent flex-nowrap">
            <TabsTrigger value="all" className="rounded-full px-6 py-2 font-black data-[state=active]:bg-primary data-[state=active]:text-white transition-all">الكل</TabsTrigger>
            <TabsTrigger value="New" className="rounded-full px-6 py-2 font-black data-[state=active]:bg-primary data-[state=active]:text-white transition-all">جديد</TabsTrigger>
            <TabsTrigger value="Pending" className="rounded-full px-6 py-2 font-black data-[state=active]:bg-primary data-[state=active]:text-white transition-all">قيد المعالجة</TabsTrigger>
            <TabsTrigger value="Escalated" className="rounded-full px-6 py-2 font-black data-[state=active]:bg-primary data-[state=active]:text-white transition-all">محال</TabsTrigger>
            <TabsTrigger value="Resolved" className="rounded-full px-6 py-2 font-black data-[state=active]:bg-primary data-[state=active]:text-white transition-all">تم الحل</TabsTrigger>
            <TabsTrigger value="Rejected" className="rounded-full px-6 py-2 font-black data-[state=active]:bg-primary data-[state=active]:text-white transition-all">مرفوض</TabsTrigger>
          </TabsList>
        </Tabs>
      </div>
             ...
             <Table>
                <TableHeader className="bg-primary text-white">
                  <TableRow className="border-none hover:bg-primary">
                    ...
                    <TableHead className="text-right h-14 font-black text-white">وسيلة الاستلام</TableHead>
                    <TableHead className="text-right h-14 font-black text-white">الحالة</TableHead>
                    <TableHead className="text-center h-14 font-black text-white pl-8">إجراء</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {filteredTickets?.map((t: any, idx: number) => (
                    <TableRow key={t.id} className={`border-b transition-colors ${idx % 2 === 0 ? 'bg-white' : 'bg-slate-50/50'} hover:bg-slate-100/50`}>
                      <TableCell className="py-5 font-black pr-8 text-right"><Badge variant="outline" className="font-mono">{t.ticketID}</Badge></TableCell>
                      <TableCell className="py-5 text-right font-bold text-slate-700">{t.customerName}</TableCell>
                      <TableCell className="py-5 text-right font-bold text-slate-500">{t.intakeMethod}</TableCell>
                  ...
                  {filteredTickets.length === 0 && (
                    <TableRow>
                      <TableCell colSpan={5} className="text-center py-28 flex flex-col items-center gap-4">
                         ...
                         <span className="font-black text-slate-400 text-lg">لا توجد بلاغات حالياً في هذا القسم</span>
                      </TableCell>
                    </TableRow>
                  )}
                </TableBody>
